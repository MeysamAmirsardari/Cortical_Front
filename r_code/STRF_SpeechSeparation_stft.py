...
# Path to sph2pipe for WSJ NIST SPHERE format (.wv1 files)
SPH2PIPE_PATH = "/mnt/localssd/diffaudio/tools/sph2pipe"


def load_audio_universal(path, sr=16000, offset=0.0, duration=1.0):
    """Load audio from WAV or NIST SPHERE format (.wv1).
    Returns: (audio_array, sample_rate)
    """
    if path.endswith('.wv1'):
        # WSJ NIST SPHERE format - use sph2pipe
        if not os.path.exists(SPH2PIPE_PATH):
            raise RuntimeError(f"sph2pipe not found at {SPH2PIPE_PATH}. "
                              f"Compile from: https://www.openslr.org/resources/3/sph2pipe_v2.5.tar.gz")
        ...
        try:
            result = subprocess.run(
                [SPH2PIPE_PATH, '-f', 'wav', path, tmp_path],
                capture_output=True, text=True
            )
            if result.returncode != 0:
                raise RuntimeError(f"sph2pipe failed: {result.stderr}")
            
            # Load with librosa (handles resampling and stereo->mono)
            audio, _ = librosa.load(tmp_path, sr=sr, mono=True, offset=offset, duration=duration)
            return audio, sr
        finally:
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
    else:
        # Standard audio file - use librosa
        return librosa.load(path=path, sr=sr, mono=True, offset=offset, duration=duration)


def get_audio_duration(path):
    """Get audio duration for WAV or NIST SPHERE format."""
    if path.endswith('.wv1'):
        # Read from SPHERE header
        try:
...
class CocktailAudioDataset(data.Dataset):
  def __init__(self, home_dir, clean_dir, noise_dir, sampling=1., snr=1., shuffle_utts=False):
    '''
    home_dir: str; path for AudSpec (can be empty "" for absolute paths)
    partition: list of tuples; home_dir/partition[i] should contains all .p files
        order: (noise, clean)
    sampling: the portion of entire data to take
    
    Supports WSJ NIST SPHERE format (.wv1) and standard WAV files.
    '''
    self.snr = snr
    # Support empty home_dir for absolute paths in file lists
    self.clean_dir = join(home_dir, clean_dir) if home_dir else clean_dir
    self.noise_dir = join(home_dir, noise_dir) if home_dir else noise_dir
    ...
    # Use universal duration function (handles .wv1 SPHERE files)
    print(f"Loading durations for {len(self.clean_utts)} clean files...")
    self.clean_utt2len = {utt: get_audio_duration(utt) for utt in self.clean_utts}
    print(f"Loading durations for {len(self.noise_utts)} noise files...")
    self.noise_utt2len = {utt: get_audio_duration(utt) for utt in self.noise_utts}
  ...
  def __getitem__(self, index_c):
    ...
    # Use universal loader (handles .wv1 SPHERE files)
    xc, _ = load_audio_universal(clean_utt, sr=16000, offset=start, duration=1.0) 
    ...
    start = random_random()*(x_len-1)
    # Use universal loader (handles .wv1 SPHERE files)
    xn, _ = load_audio_universal(noise_utt, sr=16000, offset=start, duration=1.0) 
