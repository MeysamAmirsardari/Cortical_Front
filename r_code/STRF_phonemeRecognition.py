...
convFeats{config.conv_feats}_batchSize{config.minibatch_size}/'
  ...
  def xe_loss(y, yh, y_pad):
    '''
    Cross-entropy loss - handles both batched and unbatched inputs.
    y: (batch, time) or (time,) integer labels  
    yh: (batch, time, n_phones) or (batch, time, pooled, n_phones) logits
    y_pad: dummy argument for compatibility
    '''
    # If yh has 4 dimensions (batch, time, pooled, n_phones), 
    # average over the pooled dimension first
    if len(yh.shape) == 4:
      yh = jnp.mean(yh, axis=2)  # (batch, time, n_phones)
    
    # Manual implementation for compatibility with newer JAX/optax
    log_probs = jax.nn.log_softmax(yh, axis=-1)
    # One-hot encode labels
    one_hot = jax.nn.one_hot(y, yh.shape[-1])
    # Cross-entropy: -sum(one_hot * log_probs) over classes
    return -jnp.sum(one_hot * log_probs, axis=-1)
  
  def batch_xe_loss(y, yh, y_pad):
    '''Wrapper that handles batched inputs directly'''
    return xe_loss(y, yh, y_pad)
...
class SupervisedAudioDataset(data.Dataset):
  ...
  def __init__(self, home_dir, metadata_dir, label_dir, collapse_alignment, noise_condition, snr,
    ...
    '''
    # Support empty home_dir for absolute paths
    self.metadata_dir = join(home_dir, metadata_dir) if home_dir else metadata_dir
    self.label_dir = join(home_dir, label_dir) if home_dir else label_dir
